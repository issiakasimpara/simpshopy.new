-- Script pour créer la table currency_rates et les fonctions de conversion
...
DROP POLICY IF EXISTS "Allow public read access to currency rates" ON currency_rates;
...
DROP POLICY IF EXISTS "Allow authenticated functions to manage currency rates" ON currency_rates;
...
DROP TRIGGER IF EXISTS trigger_update_currency_rate_last_updated ON currency_rates;
...
-- Insérer quelques taux de base (exemples) - L'Edge Function mettra à jour tous les taux automatiquement
...
-- Note: L'Edge Function mettra à jour automatiquement tous les taux de change pour toutes les devises du monde
-- Le Scheduler exécutera cette mise à jour quotidiennement à 6h00 UTC
...
SELECT 'Table currency_rates créée avec succès' as status;
SELECT COUNT(*) as nombre_taux FROM currency_rates;
SELECT 'Fonctions de conversion créées avec succès' as status;
